...
function ColoursCard({colourSchemes}: {colourSchemes: any}) {
  ...
  const copyTextHandler = (text: string, index: number) => {
    ...
    setTimeout(() => {setShowCopiedMessage(-1)}, 1000);
  }
    ...
    <>
      ...
      <div className="flex flex-col flex-1 w-full items-center justify-center text-center bg-white text-black rounded-3xl min-h-max py-5 shadow-gray-400 shadow-lg ">
        <div className="flex w-full flex-col items-stretch h-full pb-4">
          {colours && colours.map((colour: string, index: number) => {
            return (
              <div className="w-full flex-1 cursor-pointer overflow-hidden" style={{backgroundColor: '#' + colour}} key={index} onClick={() => copyTextHandler(colour, index)}>
                <div className="bebe text-white bg-gray-500 w-max p-1 bg-opacity-20 text-sm "> {showCopiedMessage === index ? 'Copied to Clipboard!' :  '#' + colour}</div>
              </div>
            )
          })}
        </div>
